...
func withPVC(t *testing.T, c *k8s.Client, pvc *models.PvcPublic) *models.PvcPublic {
	...
	assert.NoError(t, err, "failed to create pvc")
...
func withPV(t *testing.T, c *k8s.Client, pv *models.PvPublic) *models.PvPublic {
	...
	assert.NoError(t, err, "failed to create pv")
...
func withHPA(t *testing.T, c *k8s.Client, hpa *models.HpaPublic) *models.HpaPublic {
	...
	assert.NoError(t, err, "failed to create hpa")
...
func cleanUpPVC(t *testing.T, c *k8s.Client, id string) {
	err := c.DeletePVC(id)
	assert.NoError(t, err, "failed to delete pvc")

	deletedPVC, err := c.ReadPVC(id)
	assert.NoError(t, err, "failed to read pvc")
	assert.Nil(t, deletedPVC, "pvc still exists")

	err = c.DeletePVC(id)
	assert.NoError(t, err, "failed to delete pvc again")
}

func cleanUpPV(t *testing.T, c *k8s.Client, id string) {
	err := c.DeletePV(id)
	assert.NoError(t, err, "failed to delete pv")

	deletedPV, err := c.ReadPV(id)
	assert.NoError(t, err, "failed to read pv")
	assert.Nil(t, deletedPV, "pv still exists")

	err = c.DeletePV(id)
	assert.NoError(t, err, "failed to delete pv again")
}

func cleanUpHPA(t *testing.T, c *k8s.Client, id string) {
	err := c.DeleteHPA(id)
	assert.NoError(t, err, "failed to delete hpa")

	deletedHPA, err := c.ReadHPA(id)
	assert.NoError(t, err, "failed to read hpa")
	assert.Nil(t, deletedHPA, "hpa still exists")

	err = c.DeleteHPA(id)
	assert.NoError(t, err, "failed to delete hpa again")
}

func cleanUpSecret(t *testing.T, c *k8s.Client, id string) {
	err := c.DeleteSecret(id)
	assert.NoError(t, err, "failed to delete secret")

	deletedSecret, err := c.ReadSecret(id)
	assert.NoError(t, err, "failed to read secret")
	assert.Nil(t, deletedSecret, "secret still exists")

	err = c.DeleteSecret(id)
	assert.NoError(t, err, "failed to delete secret again")
}

func cleanUpJob(t *testing.T, c *k8s.Client, id string) {
	err := c.DeleteJob(id)
	assert.NoError(t, err, "failed to delete job")

	deletedJob, err := c.ReadJob(id)
	assert.NoError(t, err, "failed to read job")
	assert.Nil(t, deletedJob, "job still exists")

	err = c.DeleteJob(id)
	assert.NoError(t, err, "failed to delete job again")
}
