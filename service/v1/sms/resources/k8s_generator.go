...
func (kg *K8sGenerator) Deployments() []models.DeploymentPublic {
	...
	defaultLimits := models.Limits{
		CPU:    fmt.Sprintf("%fm", config.Config.Deployment.Resources.Limits.CPU),
		Memory: fmt.Sprintf("%fMi", config.Config.Deployment.Resources.Limits.RAM),
	}

	defaultRequests := models.Requests{
		CPU:    fmt.Sprintf("%fm", config.Config.Deployment.Resources.Requests.CPU),
		Memory: fmt.Sprintf("%fMi", config.Config.Deployment.Resources.Requests.RAM),
	}
...
func (kg *K8sGenerator) PVs() []models.PvPublic {
	...
	for _, v := range allVolumes {
		res = append(res, models.PvPublic{
			...
			Capacity:  fmt.Sprintf("%fGi", config.Config.Deployment.Resources.Limits.Storage),
