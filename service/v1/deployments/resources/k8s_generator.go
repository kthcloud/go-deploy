...
func (kg *K8sGenerator) Deployments() []models.DeploymentPublic {
	...
	limits := models.Limits{
		CPU:    fmt.Sprintf("%f", mainApp.CpuCores),
		Memory: fmt.Sprintf("%fGi", mainApp.RAM),
	}

	requests := models.Requests{
		CPU:    fmt.Sprintf("%f", math.Min(config.Config.Deployment.Resources.Requests.CPU, mainApp.CpuCores)),
		Memory: fmt.Sprintf("%fGi", math.Min(config.Config.Deployment.Resources.Requests.RAM, mainApp.RAM)),
	}
