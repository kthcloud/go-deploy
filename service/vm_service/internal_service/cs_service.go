...
func CreateCS(params *vmModel.CreateParams) (*CsCreated, error) {
	...
	for _, port := range params.Ports {
		...
		if useRootNetwork {
			rootPublic := createRootPortForwardingRulePublic(port.Name, csVM.ID, freePort, port.Port, port.Protocol, createDeployTags(port.Name, csVM.Name), *zone)
			_, err = createPortForwardingRule(client, vm, port.Name, rootPublic)
			if err != nil {
				return nil, makeError(err)
			}
		} else {
			...
			rootPublic := createRootPortForwardingRulePublic(
				port.Name,
				csVM.ID,
				freePort,
				port.Port,
				port.Protocol,
				createDeployTags(port.Name, csVM.Name), *zone,
			)
			_, err = createPortForwardingRule(client, vm, port.Name, rootPublic)
			if err != nil {
				return nil, makeError(err)
			...
			public := createPortForwardingRulePublic(
			...
			_, err = createPortForwardingRule(client, vm, port.Name, public)
			if err != nil {
				return nil, makeError(err)
			}
		}
...
func UpdateCS(vmID string, updateParams *vmModel.UpdateParams) error {
	...
	if updateParams.Ports != nil {
		...
		for _, port := range *updateParams.Ports {
			...
			cmp1 := createRootPortForwardingRulePublic(
				port.Name,
			...
			if !useRootNetwork {
				...
				cmp1 = createPortForwardingRulePublic(
					createNonRootPortForwardingRuleName(port.Name, network.Name),
					vm.Subsystems.CS.VM.ID,
					*vm.NetworkID,
					cmp2.IpAddressID,
				...
				cmp2, ok = ruleMap[createNonRootPortForwardingRuleName(port.Name, network.Name)]
				if !ok {
					// new rule
...
func createPortForwardingRule(client *cs.Client, vm *vmModel.VM, name string, public *csModels.PortForwardingRulePublic) (*csModels.PortForwardingRulePublic, error) {
	...
	err = vmModel.New().UpdateSubsystemByName(vm.Name, "cs", "portForwardingRuleMap", vm.Subsystems.CS.GetPortForwardingRuleMap())
	if err != nil {
		return nil, err
	}
